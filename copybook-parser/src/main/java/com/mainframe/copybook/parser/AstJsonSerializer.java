...
/**
 * Serializes a CopybookAst to JSON in a deterministic, stable format.
 * The output format matches the expected snapshot files in the test fixtures.
 * This implementation uses manual JSON construction without external dependencies.
 */
public final class AstJsonSerializer {
     ...
     */
    private void serializeNode(AstNode node, StringBuilder sb, int depth) {
        switch (node) {
            case DataItemNode item -> serializeDataItem(item, sb, depth);
            case CopyNode copy -> serializeCopyNode(copy, sb, depth);
            case ConditionNameNode cond -> serializeConditionName(cond, sb, depth);
        }
    }
     ...
     */
    private void serializeDataItem(DataItemNode item, StringBuilder sb, int depth) {
        sb.append("{\n");
        ...
        if (item.children() != null && !item.children().isEmpty()) {
            ...
            for (int i = 0; i < item.children().size(); i++) {
                indent(sb, depth + 2);
                serializeDataItem(item.children().get(i), sb, depth + 2);
    ...
    private String picToString(PicClause pic) {
        ...
        for (PicElement elem : pic.elements()) {
            switch (elem) {
                case PicSymbol s -> sb.append(s.symbol());
                case PicRepeat r -> sb.append(r.symbol()).append('(').append(r.count()).append(')');
                case PicVirtualDecimal v -> sb.append('V');
                default -> { }
            }
        }
