...
/**
 * Converts normalized program lines into a flat stream of tokens. The tokenizer
 * follows the lexical rules in copybook-parser-module-doc.md. It reports
 * LexicalError diagnostics for unknown characters but continues to produce a
 * token stream to permit best-effort parsing of partially valid copybooks.
 */
public final class Tokenizer {
	private static final Set<String> KEYWORDS = new HashSet<>(
			Set.of("PIC", "PICTURE", "USAGE", "OCCURS", "REDEFINES", "COPY", "TIMES", "VALUE", "VALUES", "THRU",
					"THROUGH", "REPLACING", "BY", "DEPENDING", "ON", "TO", "RENAMES", "IS", "COMP-3"));

	private final List<Token> tokens = new ArrayList<>();
	private final List<Diagnostic> diagnostics = new ArrayList<>();

	/**
	 * Tokenize the given normalized lines into a list of tokens terminated by EOF.
	 * Any lexical errors encountered are captured in diagnostics and do not
	 * interrupt tokenization.
	 *
		...
		for (NormalizedLine line : lines) {
			...
			while (i < len) {
				...
				if (Character.isDigit(c)) {
					...
					} else if (firstToken && (digits.length() == 2 || digits.equals("88") || digits.equals("66")
							|| digits.equals("77"))) {
				...
				// Identifier or keyword or PIC symbol (allow colon for :TAG: style
				// placeholders)
				if (Character.isLetter(c) || c == '-' || c == ':') {
					...
					// Special-case: allow compact signed picture like S9, S99, S999(…)
					// Tokenize "S" as PIC_SYMBOL and the digits as INTEGER so the PIC parser can
					// handle it.
					if (upper.length() > 1 && upper.charAt(0) == 'S') {
						String rest = upper.substring(1);
						boolean allDigits = true;
						for (int k = 0; k < rest.length(); k++) {
							if (!Character.isDigit(rest.charAt(k))) {
								allDigits = false;
								break;
							}
						}
						...
						if (allDigits) {
							// emit PIC_SYMBOL('S')
							tokens.add(new Token(TokenType.PIC_SYMBOL, "S", line.lineNumber(), 8 + start));
							// emit INTEGER(rest)
							tokens.add(new Token(TokenType.INTEGER, rest, line.lineNumber(), 8 + start + 1));
							firstToken = false;
							continue;
						}
					}
				...
				diagnostics.add(new Diagnostic("LexicalError", "LEX1", "Unrecognized character '" + c + "'",
